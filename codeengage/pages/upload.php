...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title && $code && $language) {
        $stmt = $conn->prepare('INSERT INTO snippets (user_id, title, description, code, language) VALUES (?, ?, ?, ?, ?)');
        $stmt->bind_param('issss', $user['id'], $title, $description, $code, $language);
        if ($stmt->execute()) {
        ...
        <form method="POST" class="bg-gray-800 p-6 rounded-lg shadow-lg space-y-4">
            <div class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    ...
                    <input class="w-full px-3 py-2 rounded bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-400" type="text" name="title" id="title" required />
                </div>
                <div class="w-48">
            ...
            <div>
                ...
                <textarea class="w-full px-3 py-2 rounded bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-400" name="description" id="description" rows="2"></textarea>
            </div>
            <div>
                <label class="block mb-1 font-semibold" for="code">Code</label>
                <div class="relative">
                    <textarea class="w-full px-3 py-2 rounded bg-gray-900 text-blue-100 font-mono focus:outline-none focus:ring-2 focus:ring-blue-400 min-h-[180px]" name="code" id="code" rows="8" required></textarea>
